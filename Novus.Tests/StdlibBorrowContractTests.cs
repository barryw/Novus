...
public class StdlibBorrowContractTests
{
    ...
    public void FoundationalCollectionsUseBorrowedAccessAndConsumingInsertion()
    {
        ...
        Assert.Contains("pub fn get(&self, index: u32) -> Option<&T>", vec);
        Assert.Contains("pub fn get_mut(&var self, index: u32) -> Option<&var T>", vec);
        Assert.Contains("pub fn push(&var self, consuming value: T)", vec);
        Assert.Contains("pub fn get(&self, index: u32) -> Option<&T>", deque);
        Assert.Contains("pub fn get_mut(&var self, index: u32) -> Option<&var T>", deque);
        Assert.Contains("pub fn push_back(&var self, consuming value: T)", ring);
        Assert.DoesNotContain("-> Option<*T>", ring);
        Assert.Contains("pub fn get_mut(&var self, index: u32) -> Option<&var T>", ring);
        Assert.Contains("pub fn push(&var self, consuming value: T)", small);
        Assert.Contains("pub fn set(&var self, index: u32, consuming value: T)", small);
    }
    ...
    public void CollectionViewsAndIteratorsAreOwnerTied()
    {
        ...
        Assert.Contains("pub fn get(&self, idx: u32) -> Option<&T>", freeList);
        Assert.Contains("pub fn get_mut(&var self, idx: u32) -> Option<&var T>", freeList);
    }
    ...
    public void StringAndAmigaViewsDoNotEraseOwnersIntoRawFields()
    {
        ...
        Assert.Contains("pub fn borrow_raw(ptr: *u8, len: u32)", strings);
