...
\begin{description}
    ...
    \item[\texttt{std::strings}] String handling: \texttt{Str} (borrowed), \texttt{String} (owned), \texttt{StringBuilder}, formatting
    ...
    \item[\texttt{std::ffi}] AmigaOS bindings: Exec, DOS, Graphics, Intuition, and 70+ other libraries
    ...
    \item[\texttt{std::hardware}] Hardware access: CPU detection, chipset identification, custom registers

    \item[\texttt{std::ui}] User interface: windows, screens, menus, gadgets, events

    \item[\texttt{std::graphics}] Graphics operations: drawing, sprites, copper lists, blitter

    \item[\texttt{std::audio}] Audio playback: Paula channels, MOD players, sample management
...
In Novus, the prelude gives you essentials in one import. For AmigaOS types, the \texttt{std::ffi} modules are organized by library, not by NDK header structure.
\end{comingfromc}
...
// 4. Strings
from std::strings::core import Str, String
from std::strings::format import format

// 5. Error types
from std::error::errors import DosError, ExecError, NovusError

// 6. AmigaOS structures
from std::ffi::amiga_structs import Window, Screen, RastPort

// 7. AmigaOS constants
from std::ffi::amiga_consts import MEMF_PUBLIC, MEMF_CHIP

// 8. AmigaOS library functions
from std::ffi::exec import AllocMem, FreeMem, Wait
...
from std::ffi::exec import AllocMem, FreeMem, Wait
from std::ffi::dos import Open, Close, Lock, UnLock
\end{lstlisting}
...
// use std::ffi::amiga_consts as consts
// use std::collections::hashmap as map

// Current workaround: import specific items
from std::ffi::amiga_consts import MEMF_PUBLIC, MEMF_CHIP, MEMF_FAST
...
AmigaOS memory flags are in \texttt{std::ffi::amiga\_consts}:

\begin{lstlisting}
from std::ffi::amiga_consts import MEMF_PUBLIC, MEMF_CHIP, MEMF_FAST
from std::ffi::amiga_consts import MEMF_CLEAR, MEMF_LARGEST, MEMF_TOTAL
...
The \texttt{std::ffi} module provides bindings to AmigaOS libraries.
...
\begin{lstlisting}
// Import structures
from std::ffi::amiga_structs import Window, Screen, NewWindow

// Import constants
from std::ffi::amiga_consts import WFLG_CLOSEGADGET, WFLG_DRAGBAR, WFLG_ACTIVATE
from std::ffi::amiga_consts import IDCMP_CLOSEWINDOW, IDCMP_VANILLAKEY

// Import functions
from std::ffi::intuition import OpenWindow, CloseWindow, WaitPort
...
from std::ui::window import Window, WindowBuilder
...
\begin{lstlisting}
from std::strings::core import Str, String, StringBuilder
...
\begin{lstlisting}
from std::os::dos import open, close, read, write
from std::ffi::amiga_consts import MODE_OLDFILE, MODE_NEWFILE
