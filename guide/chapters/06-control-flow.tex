...
Programs aren't just sequences of statements---they branch, loop, and make decisions. Novus provides control flow constructs that are both familiar to C programmers and enhanced with modern safety features. Pattern matching ensures you handle all cases; labeled loops prevent off-by-one errors in nested structures; and the try operator makes error handling concise without hiding the control flow.
...
\begin{comingfromc}
C allows any integer or pointer as a condition. Novus requires explicit boolean expressions:
...
\texttt{if (flags \& MASK)} & \texttt{if (flags \& MASK) != 0} \\
\end{tabular}

This catches common bugs like \texttt{if (x = 10)} (assignment instead of comparison), which would silently succeed in C but is a type error in Novus.
\end{comingfromc}
...
\begin{comingfromc}
Novus \texttt{while} loops work exactly like C's:

\begin{tabular}{ll}
\textbf{C} & \textbf{Novus} \\
\texttt{while (x > 0) \{ ... \}} & \texttt{while x > 0 \{ ... \}} \\
\texttt{do \{ ... \} while (x);} & (not supported; use \texttt{forever} + \texttt{break}) \\
\end{tabular}

Novus doesn't have \texttt{do-while} loops. Use \texttt{forever} with a conditional \texttt{break} at the end instead.
\end{comingfromc}

\subsubsection{Practical While Loop Patterns}

Common patterns using \texttt{while} loops:

\begin{lstlisting}
// Wait for hardware ready with timeout
fn wait_blitter_with_timeout(max_frames: i32) -> bool {
    var frames = 0
    while (custom.dmaconr & DMAF_BLTDONE) != 0 {
        frames++
        if frames > max_frames {
            return false  // Timeout
        }
        WaitTOF()  // Wait for frame
    }
    return true  // Blitter finished
}

// Process a linked list
fn count_nodes(mut head: *Node) -> i32 {
    var count = 0
    while head != null {
        count++
        head = head.next
...
fn read_until_newline(buffer: *var u8, max_len: i32) -> i32 {
    var pos = 0
    while pos < max_len {
        let ch = read_char()
        if ch == '\n' || ch == 0 {
...
\subsubsection{Common Forever Loop Patterns}
...
\section{Common Control Flow Patterns}
\label{sec:control-flow-patterns}

This section presents idiomatic patterns that combine multiple control flow constructs effectively.

\subsection{Early Exit Pattern}

Use early returns to handle error cases first, keeping the happy path unindented:

\begin{lstlisting}
fn process_data(input: *Data) -> Result<i32, Error> {
    // Guard clauses first
    if input == null {
        return Result::Err(Error::NullInput)
    }
    ...
    // Happy path - not nested inside if/else
    let result = compute(input)?
    return Result::Ok(result)
}
\end{lstlisting}

Compare this to the nested alternative:

\begin{lstlisting}
// Less readable - deep nesting
fn process_data_nested(input: *Data) -> Result<i32, Error> {
    if input != null {
        if input.size > 0 {
            if input.size <= MAX_SIZE {
                let result = compute(input)?
...
\subsection{Resource Acquisition Pattern}

Acquire resources with early exit, using defer for cleanup:

\begin{lstlisting}
fn safe_window_operation() -> Result<i32, Error> {
    // Acquire screen
    let screen = LockPubScreen(null)
    if screen == null {
        return Result::Err(Error::NoScreen)
    }
    defer { UnlockPubScreen(null, screen) }

    // Acquire visual info
    let vi = GetVisualInfo(screen, null)
    if vi == null {
        return Result::Err(Error::NoVisualInfo)
    }
    defer { FreeVisualInfo(vi) }

    // Acquire window
    let window = OpenWindow(screen, vi)
    if window == null {
        return Result::Err(Error::NoWindow)
    }
    defer { CloseWindow(window) }

    // All resources acquired - do work
    // Any return path will properly clean up all resources
    let result = process_window_events(window)?
    return Result::Ok(result)
}
\end{lstlisting}
...
\begin{lstlisting}
fn retry_connect(addr: *u8, max_retries: i32) -> Result<*Connection, Error> {
    var delay_ms = 100
    var attempt = 0
    ...
    while attempt < max_retries {
        ...
        match try_connect(addr) {
            ...
            Result::Err(e) => {
                ...
                // Wait before retry
                Delay(delay_ms / 20)  // Convert to ticks
                delay_ms *= 2  // Exponential backoff
            }
        }
    }

    return Result::Err(Error::MaxRetriesExceeded)
}
\end{lstlisting}

\subsection{State Machine Pattern}

Implement state machines using match:
...
fn parse_tag(input: *u8, len: i32) -> Result<Tag, ParseError> {
        ...
        state = match state {
            ParserState::Start => {
                if ch == '<' {
                    ...
                    return Result::Err(ParseError::ExpectedOpenTag)
                }
            },
            ParserState::InTag => {
                if ch == ' ' {
                    ParserState::InAttribute
                } else if ch == '>' {
                    ParserState::Done
                } else {
                    ParserState::InTag  // Keep reading tag name
                }
            },
            ParserState::InAttribute => {
                if ch == '=' {
    ...
    return build_tag(input, len)
}
\end{lstlisting}

\subsection{Search Pattern with Multiple Collections}

Search through multiple sources with early exit:

\begin{lstlisting}
fn find_resource(name: *u8) -> Option<*Resource> {
    // Check cache first
    if let res = cache_lookup(name) {
        return Option::Some(res)
    }

    // Check local directory
    if let res = local_lookup(name) {
        cache_store(name, res)
        return Option::Some(res)
    }

    // Check system paths
    for i in 0u32..system_paths.len() {
        let path = system_paths.get(i)
        if let res = path_lookup(path, name) {
            cache_store(name, res)
            return Option::Some(res)
        }
    }
...
\section{Showcase: Menu System}

This showcase demonstrates control flow constructs in a practical menu system:

\begin{lstlisting}
// menu_system_showcase.novus
// Demonstrates: conditionals, loops, match, defer, error handling
...
// Menu item types
enum MenuItem {
    Action { id: i32, label: *u8 },
    Submenu { id: i32, label: *u8, items: *MenuList },
    Separator,
}

// Menu result from user interaction
enum MenuResult {
    Selected(i32),      // Item ID selected
    Cancelled,          // User cancelled
    Timeout,            // No selection in time
}

struct MenuState {
    current_item: i32,
    is_open: bool,
    parent: Option<*MenuState>,
}
...
fn process_menu_item(item: &MenuItem, state: &var MenuState) -> MenuResult {
    match *item {
        MenuItem::Action { id, label } => {
            // Action item - return selection
            return MenuResult::Selected(id)
        },
        MenuItem::Submenu { id, label, items } => {
            // Open submenu - recursive handling
            var sub_state = MenuState::new()
            sub_state.parent = Option::Some(state as *MenuState)

            forever {
                let result = run_menu_loop(items, &var sub_state)
                match result {
                    MenuResult::Selected(sub_id) => {
                        return MenuResult::Selected(sub_id)
                    },
...
fn handle_menu_key(key: u8, state: &var MenuState, item_count: i32) -> Option<MenuResult> {
    // Use match for key handling
    match key {
        'j' => {
            // Move down, skip separators
            var next = state.current_item + 1
            while next < item_count {
                if !is_separator(next) {
                    state.current_item = next
                    break
                }
                next++
            }
        ...
        'k' => {
            ...
            var prev = state.current_item - 1
            while prev >= 0 {
                if !is_separator(prev) {
                    state.current_item = prev
                    break
                }
                prev--
            }
        ...
        '\r' => {
            ...
            return Option::Some(MenuResult::Selected(state.current_item))
        },
        27 => {
            // Escape - cancel
            return Option::Some(MenuResult::Cancelled)
        },
        _ => {
            // Unknown key
            return Option::None
...
// Main menu loop
fn run_menu_loop(items: *MenuList, state: &var MenuState) -> MenuResult {
    var timeout_counter = 0
    const MAX_TIMEOUT: i32 = 3000  // 30 seconds at 100 ticks/sec
    ...
    forever {
        ...
        let key_result = wait_for_key_with_timeout(10)
        ...
        match key_result {
            Option::Some(key) => {
                ...
                // Handle navigation
                let nav_result = handle_menu_key(key, state, items.len())

                if let result = nav_result {
                    match result {
                        MenuResult::Selected(idx) => {
                            // Process the selected item
                            let item = items.get(idx)
                            return process_menu_item(item, state)
                        },
                        MenuResult::Cancelled => {
                            return MenuResult::Cancelled
                        },
...
// Example usage
fn main() -> i32 {
    println("=== Menu System Showcase ===")

    // Build a sample menu structure
    var state = MenuState::new()

    // Simulate menu interaction
    let result = simulate_menu_interaction(&var state)

    match result {
        MenuResult::Selected(id) => {
            println(f"Selected item: {id}")
            return 0
        },
        MenuResult::Cancelled => {
...
\begin{itemize}
    \item \textbf{Match expressions} for handling enums with associated data
    \item \textbf{Forever loops} for event-driven menu processing
    \item \textbf{While loops} for skipping separators during navigation
    \item \textbf{If let} for handling optional values
    \item \textbf{Defer} for cleanup when menu closes
    \item \textbf{Nested match} for complex input handling
    \item \textbf{Early exit} with break and return
\end{itemize}
