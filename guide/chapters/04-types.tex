...
\begin{comingfromc}
No \texttt{typedef} needed! In C you often write:

\texttt{typedef struct \{ int x, y; \} Point;}

In Novus, the struct name is directly usable as a type:

\texttt{struct Point \{ x: i32, y: i32 \}}

Also note: no semicolons after field definitions, and types come \textit{after} names (\texttt{x: i32} not \texttt{int x}).
\end{comingfromc}
...
The \texttt{..expr} must appear last in the literal and copies all unspecified fields from the given expression. At least one field must be explicitly specified; you cannot use spread alone.
...
impl<T> Pair<T, T> where T: Clone {
    fn duplicate_first(self) -> Option<Pair<T, T>> {
        match self.first.clone() {
            Option::Some(copy) => {
                return Option::Some(Pair { first: self.first, second: copy })
            },
            Option::None => {
                return Option::None
            },
...
\begin{lstlisting}
trait Clone {
    fn clone(&self) -> Option<Self>  // Returns Option because allocation can fail
}
...
\texttt{Clone} & Explicit duplication (returns \texttt{Option<Self>}) \\
\texttt{Copy} & Marker trait for implicitly copyable types \\
\texttt{Eq} & Equality comparison \\
\texttt{Hash} & Hashing for use in collections \\
\texttt{From<T>} & Type conversion from \texttt{T} (method: \texttt{convert}) \\
\texttt{Default} & Provides a default value \\
\texttt{Display} & User-facing formatted output \\
\texttt{Debug} & Programmer-facing debug output \\
\hline
\end{tabular}
...
\texttt{From<T>} enables type conversions. The method is named \texttt{convert}:
...
Novus provides \texttt{if let} for binding values that pass a truthiness check. This is useful for checking pointers for null or integers for zero:
...
The condition succeeds if the value is ``truthy'' (non-null for pointers, non-zero for integers). The bound variable is available in the \texttt{if} block.

For \texttt{Option} and \texttt{Result} types, use \texttt{match} for pattern matching:
...
The preferred way to work with \texttt{Option} is pattern matching:
...
For quick checks, use the \texttt{is\_some()} and \texttt{is\_none()} methods. Note that these methods \textbf{consume} the \texttt{Option}:
...
Common \texttt{Option} methods. All methods consume \texttt{self} unless otherwise noted:
...
\hline
\texttt{is\_some(self) -> bool} & Returns \texttt{true} if \texttt{Some} (consumes) \\
\texttt{is\_none(self) -> bool} & Returns \texttt{true} if \texttt{None} (consumes) \\
\texttt{unwrap(self) -> T} & Returns value, panics on \texttt{None} \\
\texttt{unwrap\_or(self, T) -> T} & Returns value or default \\
\texttt{ok\_or(self, E) -> Result<T,E>} & Converts to \texttt{Result} \\
\hline
\end{tabular}
\caption{Common Option Methods}
\end{table}

\textbf{Note:} Because these methods consume \texttt{self}, you cannot call multiple methods on the same \texttt{Option} value. Use pattern matching when you need to inspect and then use the value.
...
The preferred way to work with \texttt{Result} is pattern matching:
...
For quick checks, \texttt{is\_ok()} and \texttt{is\_err()} consume the result:
...
Common \texttt{Result} methods. All methods consume \texttt{self}:
...
\textbf{Method} & \textbf{Description} \\
\hline
\texttt{is\_ok(self) -> bool} & Returns \texttt{true} if \texttt{Ok} (consumes) \\
\texttt{is\_err(self) -> bool} & Returns \texttt{true} if \texttt{Err} (consumes) \\
\texttt{unwrap(self) -> T} & Returns value, panics on \texttt{Err} \\
\texttt{unwrap\_or(self, T) -> T} & Returns value or default \\
\texttt{ok(self) -> Option<T>} & Converts to \texttt{Option}, discards error \\
\texttt{err(self) -> Option<E>} & Gets error as \texttt{Option} \\
\hline
\end{tabular}
