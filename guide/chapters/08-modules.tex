...
\texttt{std/amiga/raw/exec.novus} & \texttt{amiga::raw::exec} \\
\end{tabular}
\end{center}
...
from amiga::raw::consts import IDCMP_*
...
// 4. std::string - string types
from std::string import Str

// 5. std::error - error types
from std::error::errors import ExecError

// 6. amiga::raw - AmigaOS bindings
from amiga::raw::structs import Task, MsgPort
...
\begin{lstlisting}
// Import AmigaOS functions from amiga::raw
...
\begin{description}
    \item[\texttt{std::string}] String slice type (\texttt{Str})
    \item[\texttt{std::string}] Formatting and \texttt{Display} trait
\end{description}
...
\begin{description}
    ...
    \item[\texttt{amiga::sys::exec}] Synchronization and Amiga task management
\end{description}
...
\begin{description}
    \item[\texttt{amiga::sys::graphics::bitmap}] Bitmap handling
    \item[\texttt{amiga::sys::graphics::sprite}] Hardware sprites
    \item[\texttt{amiga::sys::graphics::copper}] Copper list generation
    \item[\texttt{amiga::sys::graphics::blitter}] Blitter operations
    \item[\texttt{amiga::sys::graphics::intuition}] Intuition window/screen wrappers
    \item[\texttt{amiga::sys::intuition}] Window management
    \item[\texttt{amiga::sys::intuition}] Screen management
    \item[\texttt{amiga::sys::gadtools}] Menu creation
    \item[\texttt{amiga::sys::intuition}] Event handling
\end{description}
...
\begin{description}
    \item[\texttt{amiga::raw::exec}] Exec library (memory, tasks, signals)
    \item[\texttt{amiga::raw::dos}] DOS library (files, paths, CLI)
    \item[\texttt{amiga::raw::graphics}] Graphics library (rendering, text)
    \item[\texttt{amiga::raw::intuition}] Intuition library (windows, screens)
    \item[\texttt{amiga::raw::layers}] Layers library (layer management)
    \item[\texttt{amiga::raw::gadtools}] GadTools library (GUI toolkit)
    \item[\texttt{amiga::raw::utility}] Utility library (tags, hooks)
    \item[\texttt{amiga::raw::workbench}] Workbench library (icons, AppIcons)
    \item[\texttt{amiga::raw::icon}] Icon library (icon handling)
    \item[\texttt{amiga::raw::asl}] ASL library (file/font requesters)
    \item[\texttt{amiga::raw::diskfont}] Diskfont library (font loading)
    \item[\texttt{amiga::raw::commodities}] Commodities library (hotkeys)
    \item[\texttt{amiga::raw::iffparse}] IFFParse library (IFF files)
    \item[\texttt{amiga::raw::timer\_device}] Timer device (delays, timing)
    \item[\texttt{amiga::raw::audio\_device}] Audio device (Paula access)
    \item[\texttt{amiga::raw::bsdsocket}] BSD sockets (networking)
    \item[\texttt{amiga::raw::amiga\_structs}] AmigaOS structure definitions
    \item[\texttt{amiga::raw::amiga\_consts}] AmigaOS constants
\end{description}
...
\begin{description}
    ...
    \item[\texttt{amiga::audio}] Paula audio, ProTracker player
    \item[\texttt{std::test}] Test framework
    \item[\texttt{amiga::sys}] Operating system utilities
\end{description}
